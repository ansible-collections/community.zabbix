...
short_description: Update Zabbix regular expression


description:
    - This module allows you to modify Zabbix regular expression.
...
class RegularExpression(ZabbixBase):
    def __init__(self, module, zbx=None, zapi_wrapper=None):
        ...
        if LooseVersion(self._zbx_api_version) < LooseVersion('6.0'):
            module.fail_json(msg="This module doesn't support Zabbix versions lower than 6.0")
    ...
    def _compare_expressions(self, current_expressions, future_expressions):
        if len(current_expressions) != len(future_expressions):
            return False
        for index in range(len(current_expressions)):
            current_expression = current_expressions[index]
            future_expression = future_expressions[index]

            if(current_expression['expression'] != future_expression['expression']):
                return False

            if(current_expression['expression_type'] != future_expression['expression_type']):
                return False

            if(current_expression['expression_type'] == '1'
                    and current_expression['exp_delimiter'] != future_expression['exp_delimiter']):
                return False

            if(current_expression['case_sensitive'] != future_expression['case_sensitive']):
                return False
    ...
    def update_regexp(self, current_regexp, name, test_string, expressions):
        try:
            current_expressions = sorted(current_regexp['expressions'], key=lambda x: x['expression'])
            future_expressions = self._convert_expressions_to_json(expressions)
            future_expressions = sorted(future_expressions, key=lambda x: x['expression'])
            if (current_regexp['name'] == name
                    and current_regexp['test_string'] == test_string
                    and self._compare_expressions(current_expressions, future_expressions)):
