...
* `zabbix_php_fpm_conf_user`: The user the PHP-FPM worker processes run as.
* `zabbix_php_fpm_conf_group`: The group the PHP-FPM worker processes run as.
...
* `zabbix_php_fpm_conf_listen_owner`: Default: php-fpm user. The user that owns the pool socket file, controlling which web server process can connect to the pool.
* `zabbix_php_fpm_conf_listen_group`: (Optional) The group that owns the pool socket file.
